...
/**
 * 연도별 운세 생성의 짧은 DB 트랜잭션 경계를 소유하는 협력 빈.
 ...
 */
@CommandService
class YearSelectionFortuneTransactionalStore(
    private val yearSelectionFortuneRepository: YearSelectionFortuneRepository,
) {
    /** 짧은 트랜잭션에서 (memberId, year) 락을 잡고 기존 결과를 조회한다(AI 호출 전 멱등 선조회). */
    fun findExistingWithLock(
        memberId: UUID,
        year: Int,
    ): YearSelectionFortune? {
        yearSelectionFortuneRepository.lock(memberId, year)
        return yearSelectionFortuneRepository.findByMemberIdAndYear(memberId, year)
    }

    /** 짧은 트랜잭션에서 락을 다시 잡고 재조회해, 이미 생성된 결과가 있으면 그대로 반환하고 없을 때만 저장한다(멱등 저장). */
    @ExperimentalUuidApi
    fun saveIfAbsent(yearSelectionFortune: YearSelectionFortune): YearSelectionFortune {
        yearSelectionFortuneRepository.lock(yearSelectionFortune.memberId, yearSelectionFortune.year)
        return yearSelectionFortuneRepository.findByMemberIdAndYear(yearSelectionFortune.memberId, yearSelectionFortune.year)
            ?: yearSelectionFortuneRepository.save(yearSelectionFortune)
    }
}
