    ...
    @Operation(
        ...
        description =
            """
                ...
                아직 생성되지 않은 회원은 이 호출에서 생성한 뒤 반환한다(AI 생성 시간만큼 응답이 지연될 수 있다).
                생성에 실패하면 그 실패 원인을 그대로 반환한다 — 503(회로 차단)·504(응답 지연)는 잠시 후 재시도하면 되고,
                404는 생성이 성공했는데도 조회되지 않는 예외적인 경우에만 반환된다.
            """,
    )
