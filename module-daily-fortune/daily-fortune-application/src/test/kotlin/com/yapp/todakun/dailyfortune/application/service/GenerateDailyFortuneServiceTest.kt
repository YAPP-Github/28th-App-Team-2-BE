...
@ExperimentalUuidApi
class GenerateDailyFortuneServiceTest :
    DescribeSpec({
        ...
        describe("generate") {
            context("이미 그 날짜의 오늘의 운세가 있으면") {
                it("AI를 재호출하지 않고 저장된 오늘의 운세를 그대로 반환한다") {
                    ...
                    val result = service.generate(MEMBER_ID, fortuneDate)
            ...
            context("AI가 카테고리를 정확히 5개(1개씩) 채우지 못하면") {
                it("DailyFortuneGenerationFailedException을 던지고 아무것도 저장하지 않는다") {
                    ...
                    shouldThrow<DailyFortuneGenerationFailedException> { service.generate(MEMBER_ID, fortuneDate) }
