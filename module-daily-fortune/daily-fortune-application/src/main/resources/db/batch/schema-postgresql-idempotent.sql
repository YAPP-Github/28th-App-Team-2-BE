-- spring-batch-core의 org/springframework/batch/core/schema-postgresql.sql을 기반으로,
-- 재기동 시 이미 테이블/시퀀스가 있어도 에러 없이 스킵되도록 IF NOT EXISTS를 추가한 버전.
-- (isContinueOnError로 진짜 실패까지 삼키지 않기 위해 이 스크립트로 멱등성을 확보한다.)
