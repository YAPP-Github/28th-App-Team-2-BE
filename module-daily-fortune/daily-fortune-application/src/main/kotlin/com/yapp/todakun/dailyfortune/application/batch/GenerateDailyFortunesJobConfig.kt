 ...
 * chunk size는 1로 고정한다. [CreateDailyFortunePort.create]가 회원 1명 단위로 AI 호출·저장을 자체 트랜잭션(@CommandService)으로 커밋하기 때문에,
 * chunk size를 키우면 여러 회원의 커밋이 하나의 Step 트랜잭션에 묶여 회원별 독립 커밋 격리가 깨진다.
 * AI 호출 실패는 대부분 일시적이므로 회원당 최대 [AI_CALL_RETRY_LIMIT]회 재시도하고, 그래도 실패하면 해당 회원만 skip해 배치 전체가 중단되지 않게 한다.
 * chunk size가 1이라 재시도 시 재실행 범위도 회원 1명으로 국한되어 안전하다.
 * [DailyFortuneCircuitOpenException]/[DailyFortuneTimeoutException](CircuitBreaker open/TimeLimiter timeout)은 재시도 목록에 넣지 않고 곧장 skip한다.
 * 회로가 열린 동안 재시도해 봐야 성공할 수 없어 AI 호출만 낭비되므로, 순수 일시적 실패에만 재시도를 남긴다.
 *
 * [DailyFortuneGenerationInProgressException](이미 다른 호출자가 같은 회원·날짜를 생성 중, 이슈 #90)도 같은 이유로 곧장 skip한다.
 * 해당 배치는 백오프 없이 즉시 재시도하므로 몇 초 내로 끝나지 않는 AI 생성이 그새 끝날 리 없고, 건너뛴 회원은 다음 배치나 홈 화면 자가 치유(GetTodayFortuneService)가 채운다.
 */
@Configuration
class GenerateDailyFortunesJobConfig(
    ...
    fun generateDailyFortunesStep(
        ...
        StepBuilder(GENERATE_DAILY_FORTUNES_STEP_NAME, jobRepository)
            ...
            .skip(DailyFortuneGenerationInProgressException::class.java)
