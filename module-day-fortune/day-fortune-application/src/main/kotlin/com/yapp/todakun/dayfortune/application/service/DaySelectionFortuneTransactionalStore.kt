...
/**
 * 택일 운세 생성의 짧은 DB 트랜잭션 경계를 소유하는 협력 빈.
 ...
 */
@CommandService
class DaySelectionFortuneTransactionalStore(
    private val daySelectionFortuneRepository: DaySelectionFortuneRepository,
) {
    /** 짧은 트랜잭션에서 (memberId, purpose, targetDate) 락을 잡고 기존 결과를 조회한다(AI 호출 전 멱등 선조회). */
    fun findExistingWithLock(
        ...
        return daySelectionFortuneRepository.findByMemberIdAndPurposeAndTargetDate(memberId, purpose, targetDate)
    }

    /** 짧은 트랜잭션에서 락을 다시 잡고 재조회해, 이미 생성된 결과가 있으면 그대로 반환하고 없을 때만 저장한다(멱등 저장). */
    @ExperimentalUuidApi
    fun saveIfAbsent(daySelectionFortune: DaySelectionFortune): DaySelectionFortune {
        daySelectionFortuneRepository.lock(
            daySelectionFortune.memberId,
            daySelectionFortune.purpose,
            daySelectionFortune.targetDate,
        )
        return daySelectionFortuneRepository.findByMemberIdAndPurposeAndTargetDate(
            daySelectionFortune.memberId,
            daySelectionFortune.purpose,
            daySelectionFortune.targetDate,
        ) ?: daySelectionFortuneRepository.save(daySelectionFortune)
    }
}
