 ...
 *
 * 취소 계약: 한 날짜가 실패하면 `awaitAll()`이 나머지 형제 `Deferred`를 취소한다.
 * `createOne`은 동기(블로킹) 함수이므로 코루틴 취소 신호를 직접 감지하지 못하지만, [runInterruptible]로 감싸 취소 시 실행 스레드에 `Thread.interrupt()`가 전달되게 한다.
 * 아직 시작되지 않은 형제는 즉시 건너뛰고, 실행 중인 형제는 인터럽트에 반응하는 블로킹 I/O(JDBC 소켓 읽기, HTTP 호출 등)라면 그 시점에 중단된다.
 * 다만 이는 최선 노력(best-effort)이며, 인터럽트를 무시하는 호출 구간에서는 취소 이후에도 AI 호출/저장이 끝까지 실행될 수 있다.
 * [DaySelectionFortuneTransactionalStore.saveIfAbsent]가 날짜별 독립 트랜잭션 + 멱등 저장이므로 데이터 정합성은 깨지지 않는다(부분 완료 허용).
 */
@Service
