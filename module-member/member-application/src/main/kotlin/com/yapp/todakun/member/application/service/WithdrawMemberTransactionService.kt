 ...
 * Apple OAuth 토큰 철회(외부 HTTP, 최대 8초)는 이 트랜잭션에 포함하지 않는다 — 커밋 전에 나가면
 * 이후 단계가 롤백돼도 되돌릴 수 없고, 커밋될 때까지 DB 커넥션을 불필요하게 점유하게 된다.
 * 커밋이 끝난 뒤 [WithdrawMemberService]가 반환된 회원 정보로 별도 호출한다.
 */
@CommandService
class WithdrawMemberTransactionService(
    ...
    @ExperimentalUuidApi
    fun withdraw(command: WithdrawMemberCommand): Member {
