...
class ReplaceSelfSajuChartServiceTest : DescribeSpec({
    ...
    val service =
        ReplaceSelfSajuChartService(manseryeokPort, sajuChartRepository, memberSajuLinkRepository, applicationEventPublisher)

    afterTest { clearMocks(manseryeokPort, sajuChartRepository, memberSajuLinkRepository, applicationEventPublisher) }
    ...
    describe("replace") {
        context("기존 SELF 링크가 있으면") {
            it("새로운 명식을 저장하고 기존 명식을 삭제한 뒤 SajuChartChangedEvent를 발행한다") {
