 ...
 * [data]는 값이 없으면 직렬화에서 생략된다. [reason]은 Bean Validation처럼 필드별 실패
 * 사유가 있는 경우에만 채워지며, 이때 [message]는 비운다.
 */
...
data class CommonResponse<T>(
    ...
    companion object {
        ...
        /** 에러 응답. GlobalExceptionHandler가 사용한다. */
        ...
        fun error(
            ...
            ResponseEntity
                .status(errorCode.status)
                .body(CommonResponse(success = false, code = errorCode.code, reason = reason))
