        ...
        describe("연도별 운세 생성") {
            ...
            context("이미 생성된 조합이면") {
                it("AI를 재호출하지 않고 기존 결과를 반환한다") {
                    ...
                    val first = createYearSelectionFortuneUseCase.create(YEAR, MEMBER_ID)
                    clearMocks(yearSelectionFortuneAiPort, answers = false, recordedCalls = true)

                    val second = createYearSelectionFortuneUseCase.create(YEAR, MEMBER_ID)
